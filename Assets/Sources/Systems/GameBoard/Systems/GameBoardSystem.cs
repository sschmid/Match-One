...
public sealed class GameBoardSystem : ReactiveSystem<GameEntity>, IInitializeSystem {
    ...
    public GameBoardSystem(Contexts contexts) : base(contexts.game) {
        _contexts = contexts;
        _gameBoardElements = _contexts.game.GetGroup(GameMatcher.AllOf(GameMatcher.GameBoardElement, GameMatcher.Position));
    }
        ...
        for (int row = 0; row < gameBoard.rows; row++) {
            for (int column = 0; column < gameBoard.columns; column++) {
                if (randomService.Bool(0.1f)) {
                    entityService.CreateBlocker(_contexts.game, column, row);
                } else {
                    entityService.CreateRandomPiece(_contexts.game, column, row);
                }
            }
        }
    }
