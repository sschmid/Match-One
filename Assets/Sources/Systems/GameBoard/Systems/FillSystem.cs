...
public sealed class FillSystem : ReactiveSystem<GameEntity> {
    ...
    protected override void Execute(List<GameEntity> entities) {
        ...
        for (int column = 0; column < gameBoard.columns; column++) {
            ...
            var nextRowPos = gameBoardService.GetNextEmptyRow(_contexts.game, position);
            while (nextRowPos != gameBoard.rows) {
                entityService.CreateRandomPiece(_contexts.game, column, nextRowPos);
                nextRowPos = gameBoardService.GetNextEmptyRow(_contexts.game, position);
            }
        }
    }
}
